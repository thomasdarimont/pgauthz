...
- **OPA 1.18.2** — Internal policy sidecar reachable only by pgauthzd (the sole external caller of OPA); a pgauthzd instance with `OPA_URL` set forwards the verified token to it for policy-as-code Rego (re-validating the JWT — defense in depth), and OPA calls **back** into pgauthzd's native callback for graph reads and writes
