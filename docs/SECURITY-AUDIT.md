...
### Delta findings (2026-07, v0.11 — audit attribution)

| # | Sev | Finding | Status |
|---|---|---|---|
| F16 | **Medium** | **Audit actor spoofing via `performed_by` on the public listener** (found by external review #7). The native write bodies (`/pgauthz/v1/write`, `/delete`, `/delete-user`, `/write-checked`) carry a `performed_by` field whose value took precedence over the authenticated JWT subject — on **both** listeners. The field exists *for* the internal callback listener (OPA has no end-user JWT to forward, so it asserts the subject it authenticated), but on the **public** listener any JWT-authenticated, WRITER_ROLE-holding caller could stamp another user into the **immutable audit trail** (a named asset in this threat model). No authorization impact — privileges are unaffected — but audit integrity is weakened, which matters most in regulated deployments. **Fixed:** `resolvePerformedBy` splits the trust contexts using the existing listener discriminator: on the public listener the JWT subject is authoritative (an equal body value is allowed; a differing one is **403**) unless `ALLOW_SUBJECT_OVERRIDE` is enabled — the flag that already means "callers are trusted PEPs asserting other subjects" on the decision path, so both override surfaces now share one trust switch. The callback listener is unchanged (the body value is the trusted upstream's assertion — its purpose). Table-tested across all five cases (no-value / matching / differing / override / callback). *Review #8 follow-up (fixed):* the callback listener — which has no JWT subject to fall back to — now **requires** a non-empty `performed_by` (400), closing the residual empty-attribution path; the OPA policy always passes `authn.subject_id`, so the sanctioned caller is unaffected. Future refinement (recorded, not built): distinct `actor` + `on_behalf_of` audit fields need an engine schema change. | ✅ **Fixed** |
