...
1. **Register the type** in `examples/models/demo/model.sql`:
...
- For the demo model: via `_ensure_tuple_partition` calls in `examples/models/demo/model.sql`
...
- **Opt into freshness tokens** ([ADR 0009](adr/0009-freshness-tokens.md)) — the
  ...
  X-PGAuthz-Stale` so you retry the primary. Enable with `FRESHNESS_TOKEN_KEYS`
