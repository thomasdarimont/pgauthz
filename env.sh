...
if [ -n "${COMPOSE_FILE:-}" ]; then
  ...
  # OPA / keycloak / playground overlays (persisted state, see above). The
  # DEFAULT stack is OPA-FREE (pgauthzd direct pgx); the OPA overlay
  # (compose-opa.yml) adds the OPA policy sidecar + the OPA-fronted pgauthzd
  # gateway. Implication chain: playground → keycloak → opa (keycloak/playground
  # route through pgauthzd-opa, which lives in the OPA overlay). NOTE: with OPA
  # active, OPA runs token-only (REQUIRE_TOKEN_FOR_READS=true) — the OPA/AuthZEN
  # integration suites need demo mode:
  #   PGAUTHZ_KEYCLOAK=0 PGAUTHZ_PLAYGROUND=0 ./init.sh && ./tests/test-all.sh
  # (the override is persisted, so the whole chain stays in demo mode);
  # restore with ./start.sh --cel --keycloak --playground.
  ...
  # compose-opa.yml must precede keycloak/playground: it DEFINES pgauthzd-opa,
  # which those overlays then override.
  case "${PGAUTHZ_OPA:-}" in
    1|true|yes|on)
  ...
  #   PGAUTHZ_KEYCLOAK=0 PGAUTHZ_PLAYGROUND=0 ./init.sh && ./tests/test-all.sh
  # works — init.sh persists the 0s, so test.sh (sourcing env.sh WITHOUT the
  # vars) keeps demo mode instead of flipping the stack back mid-run.
  # Restore with: ./start.sh --cel --keycloak --playground
  {
    echo "# generated by start.sh/env.sh — the overlays of the currently running stack"
    echo "# (explicit PGAUTHZ_* env vars take precedence and are then persisted here)"
