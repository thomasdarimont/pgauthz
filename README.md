...
- **Model-as-code toolchain (`pgauthzctl`)** — author models as verbatim OpenFGA DSL in git, test them in CI with YAML fixtures (a superset of OpenFGA's store-test format, plus golden explain paths), publish immutable registry versions, and plan/apply rollouts — see [`pgauthzctl/`](pgauthzctl/README.md)
...
- **pgauthzd front door — OPA opt-in** — **pgauthzd is the front door** for reads *and* writes: it validates the JWT and exposes AuthZEN 1.0 / the native `/pgauthz/v1` API, answering **directly from PostgreSQL** (conditions cover ABAC). **OPA is opt-in — the default stack is OPA-free** (enable with `./start.sh --opa` or Helm `authzen.opa.enabled`; see [ADR 0008](docs/adr/0008-opa-is-opt-in.md)). When enabled it is an **internal policy sidecar** that only pgauthzd calls (a pgauthzd instance with `OPA_URL` set forwards the verified token for policy-as-code + response caching); OPA's Rego then calls **back** into pgauthzd's native `/pgauthz/v1` API (over a shared service token, optionally mTLS) for both reads and writes
- **Policy hooks (OPA overlay, opt-in)** — when OPA is enabled, mount your own **veto rules** that ride along the standard decision/write pipeline — request-time business rules (time windows, environment gates), write governance, and compliance denylists — **global or per-store** (`authz.hooks.v1.{global,stores.<store>}.*`), composed *with* the ReBAC graph answer: a hook can only **narrow**, never widen. Hooks see a normalized, versioned input ABI (tokens invisible, server-derived time, **verified caller roles** for role-based exemptions — e.g. a Keycloak client role that lifts a veto); denials are structured + attributable; batch is evaluated per item and errors fail closed. **Search stays consistent with hooks**: enumeration is refused by default while hooks are loaded, or — with `HOOK_FILTERED_ENUMERATION` — each candidate is evaluated through the hooks so listings match per-object checks (sealed, caller-bound page cursors; fail-closed candidate cap). The veto-only contract is enforced by `scripts/validate-hooks.sh` — see [ADR 0011](docs/adr/0011-opa-policy-hooks.md) / [`examples/opa-hooks/`](examples/opa-hooks/)
- **AuthZEN 1.0 API** — standard [AuthZEN](https://openid.net/specs/authorization-api-1_0.html) API served by the one `pgauthzd` binary, capability-scoped by profile (`decision-only` | `full`, DB capability only), with OPA fronting an orthogonal `OPA_URL` flag: direct PostgreSQL (`decision-only`, service `pgauthzd-decision`) and OPA-fronted (`OPA_URL` set, service `pgauthzd-opa`); multi-tenant ready — store-scoped routes (`/stores/{store}/…`), multiple trusted issuers, and per-issuer **store / DB-role bindings** (anchored patterns, enforceable at startup)
...
|---|---|---|---|
| **Central authz service** (common) | its own database / cluster | over the wire — AuthZEN 1.0 / native `/pgauthz/v1` to pgauthzd (which, when `OPA_URL` is set, consults OPA — OPA then calls back into the native callback) | `list_objects` returns the id set; the app filters its own query by it (`WHERE id = ANY(:ids)` + wildcard flag), like an OpenFGA-style engine |
