...
- **Object wildcards** — `object_id: *` grants a relation on every object of a type (superuser/auditor-style); default-deny, models opt in — see [Object Wildcards](#object-wildcards-privileged-grants)
- **Intersection and exclusion** — rule groups with AND and BUT NOT semantics
- **Attribute-based access control (ABAC)** — per-tuple conditions (time windows, IP ranges, quotas) evaluated at check time, in SQL or [CEL](#condition-languages-lang)
