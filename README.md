...
- **Model-as-code toolchain (`authzctl`)** — author models as verbatim OpenFGA DSL in git, test them in CI with YAML fixtures (a superset of OpenFGA's store-test format, plus golden explain paths), publish immutable registry versions, and plan/apply rollouts — see [`authzctl/`](authzctl/README.md)
