...
- **Policy hooks (OPA overlay, opt-in)** — when OPA is enabled, mount your own **veto rules** that ride along the standard decision/write pipeline — request-time business rules (time windows, environment gates) and write governance — **global or per-store** (`authz.hooks.v1.{global,stores.<store>}.*`), composed *with* the ReBAC graph answer: a hook can only **narrow**, never widen. No pgauthzd config or allowlist edits; hooks see a normalized, versioned input ABI (tokens invisible, server-derived time); denials are structured + attributable; batch is evaluated per item and errors fail closed; the veto-only contract is enforced by `scripts/validate-hooks.sh` — see [ADR 0011](docs/adr/0011-opa-policy-hooks.md) / [`examples/opa-hooks/`](examples/opa-hooks/)
